...
def create_graph_from_text(paragraph, window_size) :
    """
    Takes a lemmatize piece of text and create the graph with weighted edges
    The window_size is a parameter that can be changed to enhance performance
    """
    graph_dict = {} 
    for lemmes in [paragraph[i:i+window_size] for i in xrange(len(paragraph))]:
        for j in xrange(1,len(lemmes)):
            try :
                graph_dict[lemmes[0]+"__"+lemmes[j]] += 1
            except:
                graph_dict[lemmes[0]+"__"+lemmes[j]] = 1
    my_graph = nx.DiGraph()
    for k,v in graph_dict.iteritems():
        aux = k.split('__')
        my_graph.add_weighted_edges_from([(aux[0],aux[1],v)])
    return my_graph
    
def create_global_edges(paragraph, window_size, dic) :
    """
    This is the same as create_graph_from_text but for the entire set of documents (used to compute "IDF")
    """
    for lemmes in  [paragraph[i:i+window_size] for i in xrange(len(paragraph))]:
        for j in xrange(1,len(lemmes)):
            try :
                dic[lemmes[0]+"__"+lemmes[j]] += 1
            except:
                dic[lemmes[0]+"__"+lemmes[j]] = 1
                
def compute_centrality(g):
    """
    Eigenvector centrality but this is resulting in full zero values so this one should not be used as it is
    """
    try :
        return nx.eigenvector_centrality(g, max_iter=500, tol=1e-03, weight='weight')
    except :
        return False
    
def compute_katz_centrality(g):
    """
    Katz centrality, not sure how to use it, the alpha parameters is sensitive and the algo does not converge sometimes, do not use as is
    """
    return nx.katz_centrality(g, alpha = 0.1, beta = 1,  max_iter=10000, tol=1e-03, weight='weight')

def apply_idf(dico, pageR):
    """
    Just divide the values from the global pagerank results pageR with a particular centrality result.
    Should we multiply or divide ?
    """
    for k,v in dico.iteritems() :
        aux_idf = pageR[k]
        dico[k] = v/float(aux_idf)
    return dico    

def create_graph_and_apply(paragraph, window_size, global_pagerank) :
    
    return apply_idf(nx.pagerank(create_graph_from_text(paragraph, window_size), alpha=alpha_PR, personalization=None, max_iter=max_iter_, tol=tolerance, nstart=None, weight='weight', dangling=None),global_pagerank)
    
def compute_gow(df, col_name, wind_size = 4) :
    aux_df = pd.DataFrame({})
    aux_df[col_name] = df[col_name]
    ##if the data is already preprocessed this step can be omitted
    print "Lemmatizing data"
    #aux_df["lemmatized_version"] = aux_df[col_name].apply( lambda x :remove_stop_word_and_lematize( remove_punc_and_lower(x), StopWords) )
    aux_df["savingmemory"] = aux_df[col_name].apply(lambda x : [ a for a in x.split(' ') if a not in stop_w])
    print "Done!"
    ##############################################################
    
    print "Computing global features"
    global_dict = {}
    for x  in aux_df["savingmemory"] :
        create_global_edges(x, wind_size, global_dict)
    
    global_graph = nx.DiGraph()
    for k,v in global_dict.iteritems():
        aux = k.split('__')
        global_graph.add_weighted_edges_from([(aux[0],aux[1],v)])
     
    print "Computing global Pagerank..."
    global_pagerank = nx.pagerank(global_graph, alpha=alpha_PR, personalization=None, max_iter=2000, tol=tolerance, nstart=None, weight='weight', dangling=None)
    print "Done!\n"
    
    print "Creating graphs..."  
    aux_df["savingmemory"] = aux_df["savingmemory"].apply(lambda x : create_graph_and_apply(x,window_size, global_pagerank))
    print "Done!\n"
    #print "Computing every pagerank and Combining results..."
    #aux_df["savingmemory"] = aux_df["savingmemory"].apply(lambda g : apply_idf(nx.pagerank(g, alpha=alpha_PR, personalization=None, max_iter=max_iter_, tol=tolerance, nstart=None, weight='weight', dangling=None),global_pagerank))
    #aux_df["savingmemory"] =  aux_df["savingmemory"].apply(lambda x : apply_idf(x,global_pagerank))
    #print "Done!\n"
    
    df["gow_"+col_name] = aux_df["savingmemory"]
    #return global_pagerank
    print "The PageRank computed succesfully for %s"%(col_name)
